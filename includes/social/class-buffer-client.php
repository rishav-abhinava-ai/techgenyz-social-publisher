...
final class TGSP_Buffer_Client {
	...
	public static function discover_channels() {
		...
		$organization_id = (string) get_option( 'tgsp_buffer_organization_id', '' );
		if ( '' === $organization_id && ! empty( $organizations[0]['id'] ) ) {
			$organization_id = (string) $organizations[0]['id'];
		}
		if ( '' === $organization_id ) {
			return new WP_Error( 'tgsp_buffer_no_organization', __( 'No Buffer organization was found.', 'techgenyz-social-publisher' ) );
		}
	...
	public static function publish( $platform, $caption, WP_Post $post ) {
		...
		$channel_id = trim( (string) get_option( 'tgsp_buffer_channel_' . $platform, '' ) );
		if ( '' === $channel_id ) {
			return new WP_Error( 'tgsp_buffer_channel_missing', sprintf( __( 'Map a Buffer channel for %s before sharing.', 'techgenyz-social-publisher' ), strtoupper( $platform ) ) );
		}

		$article = TGSP_Social_Payload::article_values( $post );
		if ( '' === $article['url'] ) {
			return new WP_Error( 'tgsp_invalid_article_url', __( 'The article does not have a valid HTTP or HTTPS URL for social publishing.', 'techgenyz-social-publisher' ) );
		}
		if ( 'x' === $platform ) { $caption = TGSP_Caption_Generator::prepare_x_caption( $caption, $article['url'] ); }
		$input   = array(
			'text'           => (string) $caption,
		...
		if ( empty( $payload['post']['id'] ) ) {
			$message = isset( $payload['message'] ) ? sanitize_text_field( $payload['message'] ) : __( 'Buffer did not create the post.', 'techgenyz-social-publisher' );
			return new WP_Error( 'tgsp_buffer_create_failed', $message );
		}
		return $payload['post'];
	}
	...
	public static function get_posts_status( array $remote_ids ) {
		...
		foreach ( $remote_ids as $alias => $remote_id ) {
			...
			if ( ! in_array( $alias, $allowed, true ) || '' === $remote_id || strlen( $remote_id ) > 255 || ! preg_match( '/^[A-Za-z0-9_.:\-]+$/', $remote_id ) ) { return new WP_Error( 'tgsp_buffer_invalid_post_id', __( 'A stored Buffer post ID is invalid.', 'techgenyz-social-publisher' ) ); }
		...
		foreach ( $variables as $alias => $input ) {
			...
			foreach ( $errors as $error ) { if ( isset( $error['path'][0] ) && $alias === $error['path'][0] ) { $alias_error = isset( $error['message'] ) ? sanitize_text_field( $error['message'] ) : __( 'Buffer could not retrieve this post.', 'techgenyz-social-publisher' ); break; } }
			if ( $alias_error ) { $results[ $alias ] = new WP_Error( 'tgsp_buffer_post_query_error', $alias_error ); }
			elseif ( empty( $post['id'] ) || ! hash_equals( (string) $input['id'], (string) $post['id'] ) ) { $results[ $alias ] = new WP_Error( 'tgsp_buffer_post_not_found', __( 'Buffer could not resolve the previously submitted post. No duplicate was created.', 'techgenyz-social-publisher' ) ); }
	...
	private static function request( $query, array $variables = array(), $allow_partial = false ) {
		$key = self::api_key();
		if ( '' === $key ) {
			return new WP_Error( 'tgsp_buffer_not_configured', __( 'Configure the Buffer API key first.', 'techgenyz-social-publisher' ) );
		}
		...
		if ( $code < 200 || $code >= 300 || ! is_array( $data ) ) {
			$error_code = 429 === $code ? 'tgsp_buffer_rate_limited' : 'tgsp_buffer_http_error';
			return new WP_Error( $error_code, 429 === $code ? __( 'Buffer rate limit reached. Polling has stopped; final status is not confirmed.', 'techgenyz-social-publisher' ) : sprintf( __( 'Buffer returned HTTP %d.', 'techgenyz-social-publisher' ), $code ), array( 'status_code' => $code, 'rate_limited' => 429 === $code, 'retry_after' => wp_remote_retrieve_header( $response, 'retry-after' ) ) );
		}
		if ( ! empty( $data['errors'] ) ) {
			if ( $allow_partial && ! empty( $data['data'] ) ) { $data['_errors'] = $data['errors']; return $data; }
			$message = isset( $data['errors'][0]['message'] ) ? sanitize_text_field( $data['errors'][0]['message'] ) : __( 'Buffer returned a GraphQL error.', 'techgenyz-social-publisher' );
			return new WP_Error( 'tgsp_buffer_graphql_error', $message );
		}
		return $data;
	}
	...
	private static function api_key() {
		...
		return trim( (string) get_option( 'tgsp_buffer_api_key', '' ) );
	}
}
